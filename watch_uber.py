...
def fetch_roles():
    with sync_playwright() as p:
        ...
        # Wait for list hydration; try a few selectors
        selectors = [
            "a[href*='/careers/list/']",
        ...
        # Scroll / click “Load more” defensively
        for _ in range(24):
...
def matches_target(title: str) -> bool:
    t = title.lower()
    # match “software engineer” or tokenized “swe”
...
def send_email(subject: str, body: str):
    ...
    smtp_port = int(os.getenv("SMTP_PORT") or "465")  # tolerate empty secret
...
def main():
    ...
    parser.add_argument("--test-email", action="store_true",
                        help="Send a test alert without scraping.")
    parser.add_argument("--ignore-window", action="store_true",
                        help="Run regardless of PT time window.")
    parser.add_argument("--send-all-now", action="store_true",
                        help="Email ALL current SWE jobs once (ignores de-dupe).")
    args = parser.parse_args()

    if args.test_email:
        send_email("[Uber Watch] TEST",
                   "This is a test alert from your Uber watcher. SMTP is wired.")
        print("Sent test email.")
        # continue to scrape if other flags are present
